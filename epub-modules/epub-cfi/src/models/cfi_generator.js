define(
    ['require', 'module', 'jquery', './runtime_errors', './cfi_instructions'],
    function (require, module, $, CFIErrors, CFIInstructions) {
        ...
        var Generator = {
            ...
            validateStartTextNode : function (startTextNode, characterOffset) {
                ...
                if (!startTextNode) {
                    throw new CFIErrors.NodeTypeError(startTextNode, "Cannot generate a character offset from a starting point that is not a text node");
                ...
                if (characterOffset < 0) {
                    throw new CFIErrors.OutOfRangeError(characterOffset, 0, "Character offset cannot be less than 0");
                }
                else if (characterOffset > startTextNode.nodeValue.length) {
                    throw new CFIErrors.OutOfRangeError(characterOffset, startTextNode.nodeValue.length - 1, "character offset cannot be greater than the length of the text node");
                }
            },
            ...
            validateStartElement : function (startElement) {
                ...
                if (!startElement) {
                    throw new CFIErrors.NodeTypeError(startElement, "CFI target element is undefined");
                }

                if (!(startElement.nodeType && startElement.nodeType === 1)) {
                    throw new CFIErrors.NodeTypeError(startElement, "CFI target element is not an HTML element");
                }
            },
